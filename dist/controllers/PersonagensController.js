...
class PersonagensController {
    async create(req, res) {
        const { nome, origem, nex, classe, trilha, patente, idade, forca, agilidade, inteligencia, presenca, afinidade, vigor, acrobacia, adestramento, artes, atletismo, atualidades, ciencias, crime, diplomacia, enganacao, fortitude, furtividade, iniciativa, intimidação, intuicao, investigacao, luta, medicina, ocultismo, percepcao, pilotagem, pontaria, profissao, reflexos, religiao, sobreviencia, tatica, tecnologia, vontade, id_usuario, } = req.body;
        //DEFAULT VALUES
        var pv_max;
        var pe_max;
        var ps_max;
        ...
        try {
            ...
            //CRIAR PERSONAGEM
            const newPersonagem = personagensRepository_1.personagensReposiory.create({
                nome,
                origem,
                nex,
                classe,
            ...
            await personagensRepository_1.personagensReposiory.save(newPersonagem);
    ...
    async vincularMesa(req, res) {
        ...
        try {
            ...
            //VERIFICAR MESA
            const mesa = await mesasRepository_1.mesasReposiory.findOneBy({ id: id_mesa });
            if (!mesa) {
                return res.status(404).json({ message: "A mesa não existe" });
            }
            //VINCULAR MESA
            await personagensRepository_1.personagensReposiory.update(personagem_id, {
                ...personagem,
                mesa: mesa,
            });
